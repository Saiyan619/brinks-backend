...
use crate::{dtos::{chatroom_dto::{ChatRoomsResponse, CreateRoomRequest, CreateRoomResponse, GroupChatRoomsResponse, UserChatRoomsResponse}, user_dto::{self, Response}}, errors::{ErrorMessage, HttpError}, models::ChatRoom, state::AppState, utils::middleware::JwtAuthMiddleware};
...
pub async fn create_single_room_route(Extension(app_state):Extension<Arc<AppState>>, Extension(user): Extension<JwtAuthMiddleware>, Json(body): Json<CreateRoomRequest>) -> Result<impl  IntoResponse, HttpError> {
    ...
    app_state.db_client.add_room_members(result.room_id, Some(user.user.id)).await.map_err(|_| HttpError::server_error(ErrorMessage::AddMemberFailed.return_err()))?;
    ...
    if get_user.is_verified{
        app_state.db_client.add_room_members(result.room_id, body.recipient).await.map_err(|_| HttpError::server_error(ErrorMessage::AddMemberFailedV2.return_err()))?;
